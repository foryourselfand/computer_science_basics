...
class TraceGetter:
    ...
    def get_trace(self, full_file_name: str):
        ...
        while cr != '0100':
            ...
            ip_current_bin_str = flags['IP']
            ip_current_int = int(ip_current_bin_str, 2)
            ip_previous_int = ip_current_int - 1
            ip_previous_bin = bin(ip_previous_int)
            ip_previous_bin_str = str(ip_previous_bin)[2:]
            ip_previous = Helper.from_bin_to_hex(ip_previous_bin_str, 3)

            output_new_address, output_new_code = self.get_new_address_and_new_code(ram_current)

            temp_output = [ip_previous, cr,
